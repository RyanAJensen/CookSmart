...
export default function PantryScreen() {
    ...
    useCallback(() => {
      ...
      const loadIngredients = async () => {
        try {
          ...
          if (isActive) setIngredients(pantryIngredients);
        } catch (error) {
  ...
  const renderIngredient = (ingredient: Ingredient) => {
    ...
    return (
      ...
      >
        <Card mode="outlined" style={styles.ingredientCard}>
          <Card.Content>
            <View style={styles.nameRow}>
              <Text variant="titleMedium" style={styles.ingredientName}>{ingredient.name || 'Unnamed Ingredient'}</Text>
            </View>
            <View style={styles.row}>
              <Text variant="bodySmall" style={styles.ingredientCategory}>{ingredient.category}</Text>
            </View>
            <View style={styles.nutritionBox}>
              ...
              <View style={styles.nutritionRow}><Text style={styles.nutritionLabel}>Calories:</Text><Text style={styles.nutritionValue}>{calories ? `${(mode === 'total' ? (calories * (ingredient.count || 1)).toFixed(0) : calories.toFixed(0))} kcal` : 'N/A'}</Text></View>
              <View style={styles.nutritionRow}><Text style={styles.nutritionLabel}>Protein:</Text><Text style={styles.nutritionValue}>{protein ? `${(mode === 'total' ? (protein * (ingredient.count || 1)).toFixed(1) : protein.toFixed(1))}g` : 'N/A'}</Text></View>
              <View style={styles.nutritionRow}><Text style={styles.nutritionLabel}>Carbs:</Text><Text style={styles.nutritionValue}>{carbs ? `${(mode === 'total' ? (carbs * (ingredient.count || 1)).toFixed(1) : carbs.toFixed(1))}g` : 'N/A'}</Text></View>
              <View style={styles.nutritionRow}><Text style={styles.nutritionLabel}>Fat:</Text><Text style={styles.nutritionValue}>{fat ? `${(mode === 'total' ? (fat * (ingredient.count || 1)).toFixed(1) : fat.toFixed(1))}g` : 'N/A'}</Text></View>
            </View>
            <View style={styles.amountRow}>
              ...
              {editingId === ingredient.id ? (
                ...
                <View style={styles.amountEditRow}>
                  <View style={styles.amountInputColumnWithCancel}>
                    <Text style={[styles.amountValue, styles.amountValueShifted]}>{ingredient.count || 1}</Text>
                  </View>
                  <Text style={styles.amountUnitPadded}>
                    {ingredient.serving_size && ingredient.serving_size !== 1 ? `${ingredient.serving_size} ` : ''}{ingredient.serving_unit}
                  </Text>
